...
data_transforms = {
 ...
 #      transforms.RandomAffine(degrees=30, translate=(0.12,0.12), scale=(0.9,1.1), shear=15)

    ]),
    'image': transforms.Compose([
        #torchvision.transforms.ColorJitter(brightness=(0.007, 0.007), contrast=0, saturation=0, hue=0),
...
train_loader = torch.utils.data.DataLoader(dataset = dataset, batch_size=10, num_workers=3, shuffle = True)
val_loader = torch.utils.data.DataLoader(dataset = val_ds, batch_size=7, num_workers=2, shuffle = True)

####
## Training
####
model = SwinUNETR(img_size=(512,512), in_channels=1, out_channels=3, use_checkpoint=True, spatial_dims=2)
model.to(device)
...
#scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer, T_0=30, T_mult=1, eta_min = 5e-6)
...
for epoch in range(start_epoch, max_epochs):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        outputs = model(x)
        loss = loss_function(outputs, y)# + length_regularization_loss(outputs) add if you want to use custom regularisation
        loss.backward()
    ...
    if (epoch + 1) % val_interval == 0:
        ...
        if dice_val > dice_val_best:
            ...
            torch.save(model.state_dict(),"swinunet.pth")
...
plt.ylabel('Loss')
plt.legend()
plt.savefig("swinunet_lcn_epoch_loss")
plt.clf()
...
plt.legend()
plt.savefig("swinunet_lcn_dice_score")
